...
public class RefineryCategory implements DisplayCategory<RefineryDisplay> {
    ...
    public List<Widget> setupDisplay(RefineryDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createLabel(new Point(bounds.getCenterX() - 29 - 26, bounds.getCenterY() + 20), Text.translatable("rei.dndreams.refinery.cost", display.RECIPE.powerCost)).color(0x373737).noShadow().leftAligned());
        if (display.RECIPE.dreamOnly) {
            widgets.add(Widgets.createLabel(new Point(bounds.getCenterX() - 29 - 26, bounds.getCenterY() - 31), Text.translatable("rei.dndreams.refinery.dream_only")).color(0x373737).noShadow().leftAligned());
        }
