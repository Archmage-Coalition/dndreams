...
public class SmallIslandFeature extends DefaultFeature {
	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> featurePlaceContext) {
		ChunkGenerator generator = featurePlaceContext.getGenerator();
		BlockPos center = featurePlaceContext.getOrigin();
		Random random = featurePlaceContext.getRandom();
		
		if (getYOnSurface((StructureWorldAccess) generator, center.getX(), center.getZ()) > 0) {
			return false;
		}
		...
		makeCircle((StructureWorldAccess) generator, pos, Blocks.GRASS_BLOCK.getDefaultState(), size - 1, random);
		
		pos.setY(pos.getY() - 1);
		makeCircle((StructureWorldAccess) generator, pos,Blocks.GRASS_BLOCK.getDefaultState(), size, random);
		
		pos.setY(pos.getY() - 1);
		makeCircle((StructureWorldAccess) generator, pos, Blocks.DIRT.getDefaultState(), size - 1, random);
		
		pos.setY(pos.getY() - 1);
		makeCircle((StructureWorldAccess) generator, pos, Blocks.STONE.getDefaultState(), size - 2, random);
		
		if (size > 5) {
			pos.setY(pos.getY() - 1);
			makeCircle((StructureWorldAccess) generator, pos, Blocks.STONE.getDefaultState(), size - 4, random);
		}
			...
			for (int z = -radius; z <= radius; z++) {
				...
				if (x2 + z2 <= r2 - random.nextInt(radius)) {
					if (worldState.isAir() || worldState.getMaterial().isReplaceable()) {
						...
						if (state.isOf(Blocks.GRASS_BLOCK) && !level.getBlockState(mut.up()).isAir()) {
							setState = Blocks.DIRT.getDefaultState();
						}
