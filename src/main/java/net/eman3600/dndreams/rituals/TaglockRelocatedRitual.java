...
public class TaglockRelocatedRitual extends Ritual {
    public TaglockRelocatedRitual() {
        super(250, new Ring(Ring.INNER_RING, CandleTuning.NETHER));
    }
    ...
    public boolean onCast(ServerWorld world, BlockPos pos, SoulCandleBlockEntity blockEntity) {
        ItemStack stack;
        ItemStack waystone;
        BlockPos waystonePos;
        if (!((waystone = blockEntity.getInventoryWaystone()) != null && (waystonePos = WaystoneItem.readBoundPos(waystone)) != null)) return false;

        Vec3d vec = Vec3d.ofCenter(pos);
        if ((stack = blockEntity.getInventoryTaglock()) != null && TaglockItem.isUnboundTaglock(stack)) {
            List<LivingEntity> mobs = world.getNonSpectatingEntities(LivingEntity.class, new Box(-5, -1, -5, 5, 1, 5).offset(waystonePos));
            ...
            waystone.setDamage(waystone.getDamage() + 1);
            if (waystone.getDamage() < waystone.getMaxDamage()) {
                entity = new ItemEntity(world, vec.x, vec.y + 1, vec.z, waystone);
                world.spawnEntity(entity);
            }
